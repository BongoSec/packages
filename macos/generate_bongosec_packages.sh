...
function clean_and_exit() {
    ...
    rm "${CURRENT_PATH}"/specs/bongosec-agent.pkgproj-e
...
function build_package() {
    ...
    VERSION=$(cat ${BONGOSEC_PATH}/src/VERSION | cut -d "-" -f1 | cut -c 2-)
    ...
    ${CURRENT_PATH}/package_files/build.sh "${INSTALLATION_PATH}" "${BONGOSEC_PATH}" ${JOBS}
    ...
    if packagesbuild ${AGENT_PKG_FILE} --build-folder ${DESTINATION} ; then
        ...
        pkg_name="bongosec-agent-${VERSION}-${REVISION}.${ARCH}.pkg"
        sign_pkg
...
function get_pkgproj_specs() {
    ...
    VERSION=$(< "${BONGOSEC_PATH}/src/VERSION"  cut -d "-" -f1 | cut -c 2-)

    pkg_file="specs/bongosec-agent-${ARCH}.pkgproj"
...
function main() {
    ...
    if [[ "${BUILD}" != "no" ]]; then
        check_root
        AGENT_PKG_FILE="${CURRENT_PATH}/package_files/bongosec-agent-${ARCH}.pkgproj"
        build_package
        "${CURRENT_PATH}/uninstall.sh"
    fi
    if [ "${NOTARIZE}" = "yes" ]; then
