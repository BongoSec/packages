...
clean() {
    ...
    # Clean the files
    rm -rf ${dockerfile_path}/{*.sh,*.tar.gz,bongosec-*}
...
build_deb() {
    ...
    if [ "${build_base}" == "yes" ];then
        ...
        basefile="${outdir}/bongosec-dashboard-base-${version}-${revision}-linux-x64.tar.xz"
        if ! test -f "${basefile}"; then
            echo "Did not find expected Bongosec dashboard base file: ${basefile} in output path. Exiting..."
            exit 1
        fi
    fi
...
help() {
    echo -e ""
    echo -e "NAME"
    echo -e "        $(basename "$0") - Build Bongosec dashboard base file."
    echo -e ""
    echo -e "SYNOPSIS"
    ...
    echo -e "                [Optional] Bongosec main plugin URL."
    echo -e ""
    echo -e "        -u, --updates-app <URL>"
    echo -e "                [Optional] Bongosec Check Updates plugin URL."
    echo -e ""
    echo -e "        -c, --core-app <URL>"
    echo -e "                [Optional] Bongosec Core plugin URL."
    echo -e ""
    ...
    echo -e "                [Optional] packages branch to download SPECs, not used by default."
    echo -e ""
...
main() {
    ...
    if [ ! "${plugin_main_reference}" ] && [ ! "${plugin_updates_reference}" ] && [ ! "${plugin_core_reference}" ]; then
        echo "No Bongosec plugins have been defined, ${version} pre-release development packages with revision ${revision} will be used."
