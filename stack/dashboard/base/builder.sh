...
# Bongosec dashboard base builder
...
base_dir=/opt/bongosec-dashboard-base
...
bongosec_minor=$(echo ${version} | cut -c1-3)

# Obtain the Bongosec plugin URL
if [ "${repository}" ];then
    valid_url='(https?|ftp|file)://[-[:alnum:]\+&@#/%?=~_|!:,.;]*[-[:alnum:]\+&@#/%=~_|]'
    if [[ $repository =~ $valid_url ]];then
        url="${repository}"
        if ! curl --output /dev/null --silent --head --fail "${url}"; then
            echo "The given URL to download the Bongosec plugin zip does not exist: ${url}"
            exit 1
        fi
    else
        url="https://packages-dev.bongosec.com/${repository}/ui/dashboard/bongosec-${version}-${revision}.zip"
    fi
else
    url="https://packages-dev.bongosec.com/pre-release/ui/dashboard/bongosec-${version}-${revision}.zip"
fi
...
# -----------------------------------------------------------------------------
# Customize OpenSearch Dashboards with Bongosec
# -----------------------------------------------------------------------------
...
# Replace App Title
sed -i "s|defaultValue: ''|defaultValue: \'Bongosec\'|g" ./src/core/server/opensearch_dashboards_config.js
...
sed -i 's|href:opensearchDashboardsDocLink,|href:"https://documentation.bongosec.com/'${bongosec_minor}'", iconType:darkmode?"/ui/logos/icon_dark.svg":"/ui/logos/icon_light.svg",|' ./src/core/target/public/core.entry.js

## Help link - Ask OpenSearch
sed -i 's|Ask OpenSearch|Ask Bongosec|' ./src/core/target/public/core.entry.js
sed -i 's|OPENSEARCH_DASHBOARDS_ASK_OPENSEARCH_LINK="https://github.com/opensearch-project"|=="https://bongosec.com/community/join-us-on-slack"|' ./src/core/target/public/core.entry.js

## Help link - Community
sed -i 's|OPENSEARCH_DASHBOARDS_ASK_OPENSEARCH_LINK="https://forum.opensearch.org/"|OPENSEARCH_DASHBOARDS_ASK_OPENSEARCH_LINK="https://bongosec.com/community/join-us-on-slack"|' ./src/core/target/public/core.entry.js
sed -i 's|OPENSEARCH_DASHBOARDS_ASK_OPENSEARCH_LINK="https://forum.opensearch.org/"|OPENSEARCH_DASHBOARDS_ASK_OPENSEARCH_LINK="https://bongosec.com/community/join-us-on-slack"|' ./plugins/alertingDashboards/target/public/alertingDashboards.plugin.js
sed -i 's|OPENSEARCH_DASHBOARDS_ASK_OPENSEARCH_LINK="https://forum.opensearch.org/"|OPENSEARCH_DASHBOARDS_ASK_OPENSEARCH_LINK="https://bongosec.com/community/join-us-on-slack"|' ./plugins/indexManagementDashboards/target/public/indexManagementDashboards.plugin.js
sed -i 's|OPENSEARCH_DASHBOARDS_ASK_OPENSEARCH_LINK="https://forum.opensearch.org/"|OPENSEARCH_DASHBOARDS_ASK_OPENSEARCH_LINK="https://bongosec.com/community/join-us-on-slack"|' ./plugins/notificationsDashboards/target/public/notificationsDashboards.plugin.js
sed -i 's|OPENSEARCH_DASHBOARDS_ASK_OPENSEARCH_LINK="https://forum.opensearch.org/"|OPENSEARCH_DASHBOARDS_ASK_OPENSEARCH_LINK="https://bongosec.com/community/join-us-on-slack"|' ./plugins/securityDashboards/target/public/securityDashboards.plugin.js
sed -i 's|Community|Slack channel|' ./src/core/target/public/core.entry.js
sed -i 's|href:helpSupportUrl,|href:"https://bongosec.com/community/join-us-on-slack", iconType:"logoSlack",|' ./src/core/target/public/core.entry.js

## Help link - Give feedback
sed -i 's|https://survey.opensearch.org|https://github.com/bongosec/|' src/core/server/opensearch_dashboards_config.js
...
sed -i 's|GITHUB_CREATE_ISSUE_LINK="https://github.com/opensearch-project/OpenSearch-Dashboards/issues/new/choose"|GITHUB_CREATE_ISSUE_LINK="https://github.com/bongosec/bongosec/issues/new/choose"|' ./src/core/target/public/core.entry.js
sed -i 's|"Open an issue in GitHub"|"Google group"|' ./src/core/target/public/core.entry.js
sed -i 's|href:GITHUB_CREATE_ISSUE_LINK,target:"_blank",size:"xs",iconType:"logoGithub"|href:"https://groups.google.com/forum/#!forum/bongosec/",target:"_blank",size:"xs",iconType:"/ui/logos/google_groups.svg"|' ./src/core/target/public/core.entry.js

# Custom logos
## Custom logos - Login logo
sed -i 's|props.chrome.logos.OpenSearch.url|props.http.basePath.prepend("/ui/logos/bongosec_dashboard_login_mark.svg")|g' ./plugins/securityDashboards/target/public/securityDashboards.chunk.5.js
...
# -----------------------------------------------------------------------------
# Bongosec customizations
# -----------------------------------------------------------------------------

# Add VERSION file
cp /root/VERSION .

# Add an exception for bongosec plugin install
bongosec_plugin="if (plugin.includes(\'bongosec\')) {\n    return plugin;\n  } else {\n    return \`\${LATEST_PLUGIN_BASE_URL}\/\${version}\/latest\/\${platform}\/\${arch}\/tar\/builds\/opensearch-dashboards\/plugins\/\${plugin}-\${version}.zip\`;\n  }"
sed -i "s|return \`\${LATEST_PLUGIN_BASE_URL}\/\${version}\/latest\/\${platform}\/\${arch}\/tar\/builds\/opensearch-dashboards\/plugins\/\${plugin}-\${version}.zip\`;|$bongosec_plugin|" ./src/cli_plugin/install/settings.js

# Generate build number for package.json
curl -sO ${url}
unzip *.zip 'opensearch-dashboards/bongosec/package.json'
build_number=$(jq -r '.version' ./opensearch-dashboards/bongosec/package.json | tr -d '.')$(jq -r '.revision' ./opensearch-dashboards/bongosec/package.json)
...
# Base output
cd /opt
tar -cJf bongosec-dashboard-base-"${version}"-"${revision}"-linux-${architecture}.tar.xz bongosec-dashboard-base
cp bongosec-dashboard-base-"${version}"-"${revision}"-linux-${architecture}.tar.xz /tmp/output/
