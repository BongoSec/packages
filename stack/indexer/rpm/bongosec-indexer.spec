...
# Foundation.
Summary:     Bongosec indexer is a search and analytics engine for security-related data. Documentation can be found at https://documentation.bongosec.com/current/getting-started/components/bongosec-indexer.html
...
%global INDEXER_FILE bongosec-indexer-base-%{version}-%{release}-linux-x64.tar.xz

# -----------------------------------------------------------------------------

%description
Bongosec indexer is a near real-time full-text search and analytics engine that gathers security-related data into one platform. This Bongosec central component indexes and stores alerts generated by the Bongosec server. Bongosec indexer can be configured as a single-node or multi-node cluster, providing scalability and high availability. Documentation can be found at https://documentation.bongosec.com/current/getting-started/components/bongosec-indexer.html
...
chown -R %{USER}:%{GROUP} bongosec-indexer-*/*

# Copy base files into RPM_BUILD_ROOT directory
mv bongosec-indexer-*/etc ${RPM_BUILD_ROOT}/
mv bongosec-indexer-*%{SYS_DIR}/* ${RPM_BUILD_ROOT}%{SYS_DIR}/
rm -rf bongosec-indexer-*/etc
rm -rf bongosec-indexer-*/usr
cp -pr bongosec-indexer-*/* ${RPM_BUILD_ROOT}%{INSTALL_DIR}/
...
cp %{REPO_DIR}/bongosec-certs-tool.sh ${RPM_BUILD_ROOT}%{INSTALL_DIR}/plugins/opensearch-security/tools/
cp %{REPO_DIR}/bongosec-passwords-tool.sh ${RPM_BUILD_ROOT}%{INSTALL_DIR}/plugins/opensearch-security/tools/
cp /root/documentation-templates/bongosec/config.yml ${RPM_BUILD_ROOT}%{INSTALL_DIR}/plugins/opensearch-security/tools/config.yml
...
chmod 750 ${RPM_BUILD_ROOT}/etc/init.d/bongosec-indexer
...
if [ -f %{INSTALL_DIR}/%{name}.restart ]; then
    echo -n "Starting bongosec-indexer service..."
...
%attr(740, %{USER}, %{GROUP}) %{INSTALL_DIR}/plugins/opensearch-security/tools/bongosec-certs-tool.sh
%attr(740, %{USER}, %{GROUP}) %{INSTALL_DIR}/plugins/opensearch-security/tools/bongosec-passwords-tool.sh
