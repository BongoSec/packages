...
build_rpm() {
    ...
    if [ "${build_base}" == "yes" ];then
        ...
        basefile="${outdir}/bongosec-indexer-base-${version}-${revision}-linux-x64.tar.xz"
        if ! test -f "${basefile}"; then
            echo "Did not find expected Bongosec indexer base file: ${basefile} in output path. Exiting..."
            exit 1
        fi
    fi
...
help() {
    ...
    echo "    --reference <ref>                 [Optional] packages branch to download SPECs, not used by default."
