...
test-01-manager_install-apt-error() {
    ...
    bongosec_version=1
    bongosec_revision=1
    @mockfalse apt-get install bongosec-manager=1-1 -y
    manager_install
}
...
test-02-manager_install-yum-error() {
    ...
    bongosec_version=1
    bongosec_revision=1
    @mockfalse yum install bongosec-manager-1-1 -y
    manager_install
}
...
test-03-manager_install-apt-assert() {
    apt-get install bongosec-manager=1-1 -y
    @echo 1
}
...
test-04-manager_install-yum-assert() {
    yum install bongosec-manager-1-1 -y
    @echo 1
}
...
test-05-manager_startCluster() {
    ...
    @mocktrue sed -i -e "1,20s/<name>.*<\/name>/<name>bongosec_cluster<\/name>/"  -e  "1,20s/<node_name>.*<\/node_name>/<node_name>bongosec<\/node_name>/"  -e  "1,20s/<node_type>.*<\/node_type>/<node_type>master<\/node_type>/"  -e  "1,20s/<key>.*<\/key>/<key>68b6975cf186649490e2afbc6230c317<\/key>/"  -e  "1,20s/<port>.*<\/port>/<port>1516<\/port>/"  -e  "1,20s/<bind_addr>.*<\/bind_addr>/<bind_addr>0.0.0.0<\/bind_addr>/"  -e  "1,20s/<node>.*<\/node>/<node>1.1.1.1<\/node>/"  -e  "1,20s/<hidden>.*<\/hidden>/<hidden>no<\/hidden>/"  -e  "1,20s/<disabled>.*<\/disabled>/<disabled>no<\/disabled>/"  /var/ossec/etc/ossec.conf
