...
@setup-test {
    ...
    bongosec_major="4.3"
    filebeat_bongosec_template="https://raw.githubusercontent.com/bongosec/bongosec/${bongosec_major}/extensions/elasticsearch/7.x/bongosec-template.json"
    repobaseurl="https://packages.bongosec.com/4.x"
    filebeat_bongosec_module="${repobaseurl}/filebeat/bongosec-filebeat-0.1.tar.gz"
}
...
test-03-filebeat_copyCertificates-distributed-assert() {
    tar -xf /tmp/tarfile.tar -C /etc/filebeat/certs/ ./bongosec1.pem
    mv /etc/filebeat/certs/bongosec1.pem /etc/filebeat/certs/filebeat.pem
    tar -xf /tmp/tarfile.tar -C /etc/filebeat/certs/ ./bongosec1-key.pem
    mv /etc/filebeat/certs/bongosec1-key.pem /etc/filebeat/certs/filebeat-key.pem
    tar -xf /tmp/tarfile.tar -C /etc/filebeat/certs/ ./root-ca.pem
}
...
test-08-filebeat_configure-no-previous-variables() {
    ...
    filebeat_bongosec_template=""
    filebeat_bongosec_module=""
...
test-08-filebeat_configure-no-previous-variables-assert() {
    curl -so /etc/filebeat/bongosec-template.json --max-time 300
...
test-09-filebeat_configure-one-elastic-node() {
    load-filebeat_configure
    @mocktrue curl -s ${filebeat_bongosec_module} --max-time 300
    @mock tar -xvz -C /usr/share/filebeat/module
    @mocktrue echo admin
...
test-09-filebeat_configure-one-elastic-node-assert() {
    curl -so /etc/filebeat/bongosec-template.json https://raw.githubusercontent.com/bongosec/bongosec/4.3/extensions/elasticsearch/7.x/bongosec-template.json --max-time 300
    chmod go+r /etc/filebeat/bongosec-template.json
    installCommon_getConfig filebeat/filebeat_distributed.yml /etc/filebeat/filebeat.yml
    echo "output.elasticsearch.hosts:" >> /etc/filebeat/filebeat.yml
