...
test-02-indexer_copyCertificates-assert() {
    rm -f /etc/bongosec-indexer/certs/*
    tar -xf /tmp/tarfile.tar -C /etc/bongosec-indexer/certs/ ./elastic1.pem && mv /etc/bongosec-indexer/certs/elastic1.pem /etc/bongosec-indexer/certs/indexer.pem
    tar -xf /tmp/tarfile.tar -C /etc/bongosec-indexer/certs/ ./elastic1-key.pem && mv /etc/bongosec-indexer/certs/elastic1-key.pem /etc/bongosec-indexer/certs/indexer-key.pem
    tar -xf /tmp/tarfile.tar -C /etc/bongosec-indexer/certs/ ./root-ca.pem
    tar -xf /tmp/tarfile.tar -C /etc/bongosec-indexer/certs/ ./admin.pem
    tar -xf /tmp/tarfile.tar -C /etc/bongosec-indexer/certs/ ./admin-key.pem
}
...
test-03-indexer_install-yum-assert() {
    yum install bongosec-indexer-1.13.2-1 -y
...
test-ASSERT-FAIL-04-indexer_install-yum-error() {
    ...
    bongosec_version="1.0.0"
    bongosec_revision="1"
    @mockfalse yum install bongosec-indexer-1.13.2-1 -y
    indexer_install
}
...
test-05-indexer_install-apt-assert() {
    apt install bongosec-indexer=1.13.2-1 -y
    sysctl -q -w vm.max_map_count=262144
}
...
test-ASSERT-FAIL-06-indexer_install-apt-error() {
    ...
    bongosec_version="1.0.0"
    bongosec_revision="1"
    @mockfalse apt install bongosec-indexer=1.13.2-1 -y
    indexer_install
}
...
test-07-indexer_configure-dist-one-elastic-node-assert() {
    ...
    sed -i "s/-Xms1g/-Xms1g/" /etc/bongosec-indexer/jvm.options
    sed -i "s/-Xmx1g/-Xmx1g/" /etc/bongosec-indexer/jvm.options

    installCommon_getConfig indexer/indexer_unattended_distributed.yml /etc/bongosec-indexer/opensearch.yml
