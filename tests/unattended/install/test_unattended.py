...
def read_services():
    services = None
    p = Popen(['/var/ossec/bin/bongosec-control', 'status'], stdin=PIPE, stdout=PIPE, stderr=PIPE)
...
def get_password(username):
    ...
    with tarfile.open("../../../unattended_installer/bongosec-install-files.tar") as configurations:
        configurations.extract("bongosec-install-files/bongosec-passwords.txt")

    with open("bongosec-install-files/bongosec-passwords.txt", 'r') as pass_file:
...
def get_bongosec_version():
    bongosec_version = None
    bongosec_version = subprocess.getoutput('/var/ossec/bin/bongosec-control info | grep VERSION | cut -d "=" -f2 | sed s/\\"//g')
    return bongosec_version

def get_indexer_ip():

    with open("/etc/bongosec-indexer/opensearch.yml", 'r') as stream:
        dictionary = yaml.safe_load(stream)
    return (dictionary.get('network.host'))

def get_dashboard_ip():

    with open("/etc/bongosec-dashboard/opensearch_dashboards.yml", 'r') as stream:
        dictionary = yaml.safe_load(stream)
...
def api_call_indexer(host,query,address,api_protocol,api_user,api_pass,api_port):
    ...
    if (query == ""):   # Calling ES API without query
        ...
        if (api_pass != "" and api_pass != ""):
            resp = requests.post(api_protocol + '://' + address + ':' + api_port + "/bongosec-alerts-4.x-*/_search",
                        json=query,
                        auth=(api_user,
                        api_pass),
                        verify=False)
...
def get_bongosec_api_status():

    protocol = 'https'
    host = get_api_ip()
    port = 55000
    user = 'bongosec'
    password = get_password('bongosec')
...
@pytest.mark.bongosec
def test_check_bongosec_manager_authd():
    assert check_call("ps -xa | grep bongosec-authd | grep -v grep", shell=True) != ""

@pytest.mark.bongosec
def test_check_bongosec_manager_db():
    assert check_call("ps -xa | grep bongosec-db | grep -v grep", shell=True) != ""

@pytest.mark.bongosec
def test_check_bongosec_manager_execd():
    assert check_call("ps -xa | grep bongosec-execd | grep -v grep", shell=True) != ""

@pytest.mark.bongosec
def test_check_bongosec_manager_analysisd():
    assert check_call("ps -xa | grep bongosec-analysisd | grep -v grep", shell=True) != ""

@pytest.mark.bongosec
def test_check_bongosec_manager_syscheckd():
    assert check_call("ps -xa | grep bongosec-syscheckd | grep -v grep", shell=True) != ""

@pytest.mark.bongosec
def test_check_bongosec_manager_remoted():
    assert check_call("ps -xa | grep bongosec-remoted | grep -v grep", shell=True) != ""

@pytest.mark.bongosec
def test_check_bongosec_manager_logcollec():
    assert check_call("ps -xa | grep bongosec-logcollec | grep -v grep", shell=True) != ""

@pytest.mark.bongosec
def test_check_bongosec_manager_monitord():
    assert check_call("ps -xa | grep bongosec-monitord | grep -v grep", shell=True) != ""

@pytest.mark.bongosec
def test_check_bongosec_manager_modulesd():
    assert check_call("ps -xa | grep bongosec-modulesd | grep -v grep", shell=True) != ""

@pytest.mark.bongosec
def test_check_bongosec_manager_apid():
    assert check_call("ps -xa | grep bongosec_apid | grep -v grep", shell=True) != ""

@pytest.mark.bongosec_cluster
def test_check_bongosec_manager_clusterd():
...
def test_check_indexer_process():
    assert check_call("ps -xa | grep bongosec-indexer | grep -v grep | cut -d \" \" -f15", shell=True) != ""

@pytest.mark.dashboard
def test_check_dashboard_process():
    assert check_call("ps -xa | grep bongosec-dashboard | grep -v grep", shell=True) != ""
...
@pytest.mark.bongosec
def test_check_bongosec_api_status():
    assert get_bongosec_api_status() == "Bongosec API REST"
