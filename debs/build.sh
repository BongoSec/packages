...
if [ ${build_target} = "api" ]; then
    if [ "${local_source_code}" = "no" ]; then
        curl -sL https://github.com/bongosec/bongosec-api/tarball/${bongosec_branch} | tar zx
    fi
    bongosec_version="$(grep version bongosec*/package.json | cut -d '"' -f 4)"
else
    if [ "${local_source_code}" = "no" ]; then
        curl -sL https://github.com/bongosec/bongosec/tarball/${bongosec_branch} | tar zx
    fi
    bongosec_version="$(cat bongosec*/src/VERSION | cut -d 'v' -f 2)"
fi

# Build directories
build_dir=/build_bongosec
package_full_name="bongosec-${build_target}-${bongosec_version}"
sources_dir="${build_dir}/${build_target}/${package_full_name}"

mkdir -p ${build_dir}/${build_target}
cp -R bongosec* ${build_dir}/${build_target}/bongosec-${build_target}-${bongosec_version}

if [ "${use_local_specs}" = "no" ]; then
    curl -sL https://github.com/bongosec/packages/tarball/${bongosec_packages_branch} | tar zx
    package_files="bongosec*/debs"
...
if [[ "${future}" == "yes" ]]; then
    # MODIFY VARIABLES
    base_version=$bongosec_version
    MAJOR=$(echo $base_version | cut -dv -f2 | cut -d. -f1)
    MINOR=$(echo $base_version | cut -d. -f2)
    bongosec_version="${MAJOR}.30.0"
    file_name="bongosec-${build_target}-${bongosec_version}-${package_release}"
    old_name="bongosec-${build_target}-${base_version}-${package_release}"
    package_full_name=bongosec-${build_target}-${bongosec_version}
    old_package_name=bongosec-${build_target}-${base_version}
    mv "${build_dir}/${build_target}/${old_package_name}" "${build_dir}/${build_target}/${package_full_name}"
    sources_dir="${build_dir}/${build_target}/${package_full_name}"

    # PREPARE FUTURE SPECS AND SOURCES
    find "${build_dir}/${package_name}" "${specs_path}" \( -name "*VERSION*" -o -name "*changelog*" \) -exec sed -i "s/${base_version}/${bongosec_version}/g" {} \;
    sed -i "s/\$(VERSION)/${MAJOR}.${MINOR}/g" "${build_dir}/${build_target}/${package_full_name}/src/Makefile"
    sed -i "s/${base_version}/${bongosec_version}/g" "${build_dir}/${build_target}/${package_full_name}/src/init/bongosec-server.sh"
    sed -i "s/${base_version}/${bongosec_version}/g" "${build_dir}/${build_target}/${package_full_name}/src/init/bongosec-client.sh"
    sed -i "s/${base_version}/${bongosec_version}/g" "${build_dir}/${build_target}/${package_full_name}/src/init/bongosec-local.sh"
fi
...
if [ "${build_target}" == "api" ]; then
    sed -i "s:DIR=\"/var/ossec\":DIR=\"${dir_path}\":g" ${sources_dir}/debian/bongosec-api.init
...
deb_file="bongosec-${build_target}_${bongosec_version}-${package_release}"
if [[ "${architecture_target}" == "ppc64le" ]]; then
...
mv ${pkg_path}/${deb_file} /var/local/bongosec
