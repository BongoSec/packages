...
function pack_wpk() {
    ...
    docker run -t --rm -v ${KEYDIR}:/etc/bongosec:Z -v ${DESTINATION}:/var/local/bongosec:Z -v ${PKG_PATH}:/var/pkg:Z \
...
function build_wpk_linux() {
    ...
    docker run -t --rm -v ${KEYDIR}:/etc/bongosec:Z -v ${DESTINATION}:/var/local/bongosec:Z \
        -v ${CHECKSUMDIR}:/var/local/checksum:Z \
