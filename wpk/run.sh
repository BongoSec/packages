...
main() {
    ...
    BONGOSEC_VERSION=$(cat src/VERSION)
    MAJOR=$(echo ${BONGOSEC_VERSION} | cut -dv -f2 | cut -d. -f1)
    MINOR=$(echo ${BONGOSEC_VERSION} | cut -d. -f2)
...
clean() {
    ...
    rm -rf src/{remoted,reportd,shared,unit_tests,bongosec_db}

    # Clean syscheckd folder
    find src/syscheckd -type f -not -name "bongosec-syscheckd" -not -name "libfimdb.dylib" -not -name "libfimdb.so" -delete
