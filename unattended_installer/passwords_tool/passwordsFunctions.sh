...
function passwords_changePassword() {
    ...
    if [ -n "${changeall}" ]; then
        if [ -n "${indexer_installed}" ] && [ -z ${no_indexer_backup} ]; then
            eval "mkdir /etc/bongosec-indexer/backup/ ${debug}"
            eval "cp /etc/bongosec-indexer/opensearch-security/* /etc/bongosec-indexer/backup/ ${debug}"
            passwords_createBackUp
        fi

        for i in "${!passwords[@]}"
        do
            if [ -n "${indexer_installed}" ] && [ -f "/etc/bongosec-indexer/backup/internal_users.yml" ]; then
                awk -v new=${hashes[i]} 'prev=="'${users[i]}':"{sub(/\042.*/,""); $0=$0 new} {prev=$1} 1' /etc/bongosec-indexer/backup/internal_users.yml > internal_users.yml_tmp && mv -f internal_users.yml_tmp /etc/bongosec-indexer/backup/internal_users.yml
            fi
        ...
        if [ -n "${indexer_installed}" ] && [ -f "/etc/bongosec-indexer/backup/internal_users.yml" ]; then
            awk -v new='"'"${hash}"'"' 'prev=="'${nuser}':"{sub(/\042.*/,""); $0=$0 new} {prev=$1} 1' /etc/bongosec-indexer/backup/internal_users.yml > internal_users.yml_tmp && mv -f internal_users.yml_tmp /etc/bongosec-indexer/backup/internal_users.yml
        fi
    ...
    if [ "${nuser}" == "admin" ] || [ -n "${changeall}" ]; then
        if [ -n "${filebeat_installed}" ]; then
            ...
            eval "/var/ossec/bin/bongosec-keystore -f indexer -k password -v ${adminpass}"
            common_logger -nl $"The new password for Filebeat is ${adminpass}"

            passwords_restartService "bongosec-manager"
        fi
    fi

    if [ "$nuser" == "kibanaserver" ] || [ -n "$changeall" ]; then
        if [ -n "${dashboard_installed}" ] && [ -n "${dashpass}" ]; then
            if /usr/share/bongosec-dashboard/bin/opensearch-dashboards-keystore --allow-root list | grep -q opensearch.password; then
                eval "echo ${dashpass} | /usr/share/bongosec-dashboard/bin/opensearch-dashboards-keystore --allow-root add -f --stdin opensearch.password ${debug_pass} > /dev/null 2>&1"
            else
                bongosecdashold=$(grep "password:" /etc/bongosec-dashboard/opensearch_dashboards.yml )
                rk="opensearch.password: "
                bongosecdashold="${bongosecdashold//$rk}"
                conf="$(awk '{sub("opensearch.password: .*", "opensearch.password: '"${dashpass}"'")}1' /etc/bongosec-dashboard/opensearch_dashboards.yml)"
                echo "${conf}" > /etc/bongosec-dashboard/opensearch_dashboards.yml
            fi
            passwords_restartService "bongosec-dashboard"
...
function passwords_changePasswordApi() {
    ...
    if [ -f "/usr/share/bongosec-dashboard/data/bongosec/config/bongosec.yml" ]; then
        bongosec_yml_user=$(awk '/- default:/ {found=1} found && /username:/ {print $2}' /usr/share/bongosec-dashboard/data/bongosec/config/bongosec.yml)
    fi
    if [ -n "${changeall}" ]; then
        for i in "${!api_passwords[@]}"; do
            if [ -n "${bongosec_installed}" ]; then
                ...
                if [ -z "${AIO}" ] && [ -z "${indexer}" ] && [ -z "${dashboard}" ] && [ -z "${bongosec}" ] && [ -z "${start_indexer_cluster}" ]; then
                    common_logger -nl $"The password for Bongosec API user ${api_users[i]} is ${api_passwords[i]}"
                fi
            fi
            if [ "${api_users[i]}" == "${bongosec_yml_user}" ] && [ -n "${dashboard_installed}" ]; then
        ...
        if [ -n "${bongosec_installed}" ]; then
            ...
            eval 'common_curl -s -k -X PUT -H \"Authorization: Bearer $TOKEN_API\" -H \"Content-Type: application/json\" -d "$BONGOSEC_PASS_API" "https://localhost:55000/security/users/${user_id}" -o /dev/null --max-time 300 --retry 5 --retry-delay 5 --fail'
            if [ -z "${AIO}" ] && [ -z "${indexer}" ] && [ -z "${dashboard}" ] && [ -z "${bongosec}" ] && [ -z "${start_indexer_cluster}" ]; then
                common_logger -nl $"The password for Bongosec API user ${nuser} is ${password}"
            fi
        fi
        if [ "${nuser}" == "${bongosec_yml_user}" ] && [ -n "${dashboard_installed}" ]; then
...
function passwords_changeDashboardApiPassword() {
    ...
    until [ -n "${file_exists}" ] || [ "${j}" -eq "12" ]; do
        if [ -f "/usr/share/bongosec-dashboard/data/bongosec/config/bongosec.yml" ]; then
            eval "sed -i 's|password: .*|password: \"${1}\"|g' /usr/share/bongosec-dashboard/data/bongosec/config/bongosec.yml ${debug}"
            ...
            if [ -z "${AIO}" ] && [ -z "${indexer}" ] && [ -z "${dashboard}" ] && [ -z "${bongosec}" ] && [ -z "${start_indexer_cluster}" ]; then
                if [ -z "${bongosec_installed}" ]; then
                    common_logger "Updated bongosec-wui user password in bongosec dashboard to '${1}'."
                else
                    common_logger "Updated bongosec-wui user password in bongosec dashboard."
                fi
            fi
            file_exists=1
        fi
...
function passwords_checkUser() {
    ...
    if { [ -n "${adminUser}" ] && [ -n "${adminPassword}" ]; } || { [ -z "${bongosec_installed}" ] && [ -n "${dashboard_installed}" ]; }; then
...
function passwords_createBackUp() {
    ...
    if [ -z "${indexer_installed}" ] && [ -z "${dashboard_installed}" ] && [ -z "${filebeat_installed}" ]; then
        common_logger -e "Cannot find Bongosec indexer, Bongosec dashboard or Filebeat on the system."
        exit 1;
    else
        if [ -n "${indexer_installed}" ]; then
            capem=$(grep "plugins.security.ssl.transport.pemtrustedcas_filepath: " /etc/bongosec-indexer/opensearch.yml )
    ...
    if [ ! -d "/etc/bongosec-indexer/backup" ]; then
        eval "mkdir /etc/bongosec-indexer/backup ${debug}"
    fi
    eval "JAVA_HOME=/usr/share/bongosec-indexer/jdk/ OPENSEARCH_CONF_DIR=/etc/bongosec-indexer /usr/share/bongosec-indexer/plugins/opensearch-security/tools/securityadmin.sh -backup /etc/bongosec-indexer/backup -icl -p 9200 -nhnv -cacert ${capem} -cert ${adminpem} -key ${adminkey} -h ${IP} ${debug}"
    ...
    common_logger -d "Password backup created in /etc/bongosec-indexer/backup."
...
function passwords_generateHash() {
    ...
    if [ -n "${changeall}" ]; then
        ...
        for i in "${!passwords[@]}"
        do
            nhash=$(bash /usr/share/bongosec-indexer/plugins/opensearch-security/tools/hash.sh -p "${passwords[i]}" 2>&1 | grep -A 2 'issues' | tail -n 1)
            if [  "${PIPESTATUS[0]}" != 0  ]; then
        ...
        hash=$(bash /usr/share/bongosec-indexer/plugins/opensearch-security/tools/hash.sh -p "${password}" 2>&1 | grep -A 2 'issues' | tail -n 1)
        if [  "${PIPESTATUS[0]}" != 0  ]; then
            common_logger -e "Hash generation failed."
            if [[ $(type -t installCommon_rollBack) == "function" ]]; then
                installCommon_rollBack
...
function passwords_generatePasswordFile() {
    ...
    user_description=(
        "Admin user for the web user interface and Bongosec indexer. Use this user to log in to Bongosec dashboard"
        "Bongosec dashboard user for establishing the connection with Bongosec indexer"
        "Regular Dashboard user, only has read permissions to all indices and all permissions on the .kibana index"
        "Filebeat user for CRUD operations on Bongosec indices"
        "User with READ access to all indices"
        "User with permissions to perform snapshot and restore operations"
        "Admin user used to communicate with Bongosec API"
        "Regular user to query Bongosec API"
    )
    api_user_description=(
        "Password for bongosec API user"
        "Password for bongosec-wui API user"
    )
...
function passwords_getApiToken() {
    ...
    while [[ "${TOKEN_API}" =~ "Bongosec Internal Error" ]] && [ "${retries}" -lt "${max_internal_error_retries}" ]
    do
    ...
    if [[ ${TOKEN_API} =~ "Bongosec Internal Error" ]]; then
...
function passwords_getApiUserId() {
    ...
    if [ "${user_id}" == "noid" ]; then
        common_logger -e "User ${1} is not registered in Bongosec API"
        if [[ $(type -t installCommon_rollBack) == "function" ]]; then
                installCommon_rollBack
        fi
        exit 1
...
function passwords_getNetworkHost() {
    ...
    IP=$(grep -hr "^network.host:" /etc/bongosec-indexer/opensearch.yml)
    NH="network.host: "
    IP="${IP//$NH}"
...
function passwords_readUsers() {
    ...
    if [ -n "${indexer_installed}" ]; then
        ...
        susers=$(grep -B 1 hash: /etc/bongosec-indexer/opensearch-security/internal_users.yml | grep -v hash: | grep -v "-" | awk '{ print substr( $0, 1, length($0)-1 ) }')
        mapfile -t users <<< "${susers[@]}"
    elif  [ -n "${bongosec_installed}" ]; then
...
function passwords_runSecurityAdmin() {
    ...
    eval "OPENSEARCH_CONF_DIR=/etc/bongosec-indexer /usr/share/bongosec-indexer/plugins/opensearch-security/tools/securityadmin.sh -f /etc/bongosec-indexer/backup/internal_users.yml -t internalusers -p 9200 -nhnv -cacert ${capem} -cert ${adminpem} -key ${adminkey} -icl -h ${IP} ${debug}"
    if [  "${PIPESTATUS[0]}" != 0  ]; then
        common_logger -e "Could not load the changes."
        exit 1;
    fi
    eval "cp /etc/bongosec-indexer/backup/internal_users.yml /etc/bongosec-indexer/opensearch-security/internal_users.yml"
    eval "rm -rf /etc/bongosec-indexer/backup/ ${debug}"

    if [[ -n "${nuser}" ]] && [[ -n ${autopass} ]]; then
        common_logger -nl "The password for user ${nuser} is ${password}"
        common_logger -w "Password changed. Remember to update the password in the Bongosec dashboard, Bongosec server, and Filebeat nodes if necessary, and restart the services."
    fi
    ...
    if [ -n "${changeall}" ]; then
        if [ -z "${AIO}" ] && [ -z "${indexer}" ] && [ -z "${dashboard}" ] && [ -z "${bongosec}" ] && [ -z "${start_indexer_cluster}" ]; then
            ...
            common_logger -w "Bongosec indexer passwords changed. Remember to update the password in the Bongosec dashboard, Bongosec server, and Filebeat nodes if necessary, and restart the services."
        else
...
function passwords_updateInternalUsers() {
    ...
    internal_users_backup_path="/etc/bongosec-indexer/internalusers-backup"
    passwords_getNetworkHost
    passwords_createBackUp

    eval "mkdir -p ${internal_users_backup_path} ${debug}"
    eval "cp /etc/bongosec-indexer/backup/internal_users.yml ${internal_users_backup_path}/internal_users_${backup_datetime}.yml.bkp ${debug}"
    ...
    eval "chown -R bongosec-indexer:bongosec-indexer ${internal_users_backup_path} ${debug}"
    common_logger "A backup of the internal users has been saved in the /etc/bongosec-indexer/internalusers-backup folder."

    eval "cp /etc/bongosec-indexer/backup/internal_users.yml /etc/bongosec-indexer/opensearch-security/internal_users.yml ${debug}"
    eval "rm -rf /etc/bongosec-indexer/backup/ ${debug}"
