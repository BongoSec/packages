...
function getHelp() {
    ...
    echo -e ""
    echo -e "NAME"
    echo -e "        $(basename "${0}") - Manage passwords for Bongosec indexer users."
    echo -e ""
    echo -e "SYNOPSIS"
    ...
    echo -e "DESCRIPTION"
    echo -e "        -a,  --change-all"
    echo -e "                Changes all the Bongosec indexer and Bongosec API user passwords and prints them on screen."
    echo -e "                To change API passwords -au|--admin-user and -ap|--admin-password are required."
    echo -e ""
    echo -e "        -A,  --api"
    echo -e "                Change the Bongosec API password."
    echo -e "                Requires -u|--user, and -p|--password, -au|--admin-user and -ap|--admin-password."
    echo -e ""
    echo -e "        -au,  --admin-user <adminUser>"
    echo -e "                Admin user for Bongosec API, Required to change Bongosec API passwords."
    echo -e "                Requires -A|--api."
    echo -e ""
    echo -e "        -ap,  --admin-password <adminPassword>"
    echo -e "                Password for Bongosec API admin user, Required to change Bongosec API passwords."
    echo -e "                Requires -A|--api."
    echo -e ""
    echo -e "        -u,  --user <user>"
    ...
    echo -e ""
    echo -e "        -f,  --file <bongosec-passwords.txt>"
    echo -e "                Changes the passwords for the ones given in the file."
    echo -e ""
    echo -e "                Bongosec indexer users must have this format:"
    echo -e ""
    echo -e "                    # Description"
    echo -e "                      indexer_username: <user>"
    echo -e "                      indexer_password: <password>"
    echo -e ""
    echo -e "                Bongosec API users must have this format:"
    echo -e ""
    echo -e "                    # Description"
    echo -e "                      api_username: <user>"
    echo -e "                      api_password: <password>"
    echo -e ""
    echo -e "        -gf, --generate-file <bongosec-passwords.txt>"
...
function main() {
    ...
    if [ -n "${1}" ]; then
        ...
        export JAVA_HOME=/usr/share/bongosec-indexer/jdk/
        ...
        if [ -n "${nuser}" ]; then
            if [ -n "${adminUser}" ] && [ -n "${adminPassword}" ]; then
                ...
                passwords_getApiIds
            elif [ -z "${bongosec_installed}" ] && [ -n "${dashboard_installed}" ]; then
                passwords_readDashboardUsers
            elif [ -n "${indexer_installed}" ] || [ -n "${bongosec_installed}" ]; then
                passwords_readUsers
            fi
            passwords_checkUser
        fi
        ...
        if [ -n "${changeall}" ] || [ -n "${p_file}" ]; then
            if [ -n "${indexer_installed}" ] || [ -n "${bongosec_installed}" ]; then
                passwords_readUsers
            fi
            ...
            if [ -n "${bongosec_installed}" ]; then
                if [ -n "${adminUser}" ] && [ -n "${adminPassword}" ]; then
                    ...
                    common_logger "Bongosec API admin credentials not provided, Bongosec API passwords not changed."
                fi
        ...
        if [ -z "${indexer_installed}" ] && { [ -n "${bongosec_installed}" ] || [ -n "${dashboard_installed}" ]; }; then
            passwords_changePassword
        fi
