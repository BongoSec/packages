...
function getHelp() {
    ...
    echo -e ""
    echo -e "NAME"
    echo -e "        bongosec-cert-tool.sh - Manages the creation of certificates of the Bongosec components."
    echo -e ""
    echo -e "SYNOPSIS"
    echo -e "        bongosec-cert-tool.sh [OPTIONS]"
    echo -e ""
    echo -e "DESCRIPTION"
    ...
    echo -e ""
    echo -e "        -wd,  --bongosec-dashboard-certificates </path/to/root-ca.pem> </path/to/root-ca.key>"
    echo -e "                Creates the Bongosec dashboard certificates, add root-ca.pem and root-ca.key."
    echo -e ""
    echo -e "        -wi,  --bongosec-indexer-certificates </path/to/root-ca.pem> </path/to/root-ca.key>"
    echo -e "                Creates the Bongosec indexer certificates, add root-ca.pem and root-ca.key."
    echo -e ""
    echo -e "        -ws,  --bongosec-server-certificates </path/to/root-ca.pem> </path/to/root-ca.key>"
    echo -e "                Creates the Bongosec server certificates, add root-ca.pem and root-ca.key."
    echo -e ""
    echo -e "        -tmp,  --cert_tmp_path </path/to/tmp_dir>"
    echo -e "                Modifies the default tmp directory (/tmp/bongosec-ceritificates) to the specified one."
    ...
    if [ -n "${1}" ]; then
        ...
        do
            ...
            "-v"|"--verbose")
                ...
                shift 1
                ;;
            "-wd"|"--bongosec-dashboard-certificates")
                if [[ -z "${2}" || -z "${3}" ]]; then
                    common_logger -e "Error on arguments. Probably missing </path/to/root-ca.pem> </path/to/root-ca.key> after -wd|--bongosec-dashboard-certificates"
                    getHelp
                    exit 1
                else
                    cdashboard=1
                ...
                ;;
            "-wi"|"--bongosec-indexer-certificates")
                if [[ -z "${2}" || -z "${3}" ]]; then
                    common_logger -e "Error on arguments. Probably missing </path/to/root-ca.pem> </path/to/root-ca.key> after -wi|--bongosec-indexer-certificates"
                    getHelp
                    exit 1
                else
                    cindexer=1
                ...
                ;;
            "-ws"|"--bongosec-server-certificates")
                if [[ -z "${2}" || -z "${3}" ]]; then
                    common_logger -e "Error on arguments. Probably missing </path/to/root-ca.pem> </path/to/root-ca.key> after -ws|--bongosec-server-certificates"
                    getHelp
                    exit 1
                else
                    cserver=1
        ...
        if [[ -n "${all}" ]]; then
            ...
            if cert_generateIndexercertificates; then
                common_logger "Bongosec indexer certificates created."
            fi
            if cert_generateFilebeatcertificates; then
                common_logger "Bongosec Filebeat certificates created."
            fi
            if cert_generateDashboardcertificates; then
                common_logger "Bongosec dashboard certificates created."
            fi
            cert_cleanFiles
            cert_setDirectory
        fi
