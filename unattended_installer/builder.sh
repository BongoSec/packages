...
# Tool to create bongosec-install.sh, bongosec-cert-tool.sh
# and bongosec-passwords-tool.sh
...
function getHelp() {
    ...
    echo -e "DESCRIPTION"
    echo -e "        -i,  --installer"
    echo -e "                Builds the unattended installer single file bongosec-install.sh"
    echo -e ""
    echo -e "        -c,  --cert-tool"
    echo -e "                Builds the certificate creation tool bongosec-cert-tool.sh"
    echo -e ""
    ...
    echo -e "                Builds the password creation and modification tool bongosec-password-tool.sh"
    echo -e ""
    echo -e "        -h,  --help"
    echo -e "                Shows help."
    exit 1
...
function buildInstaller() {
    ...
    output_script_path="${base_path_builder}/bongosec-install.sh"
...
# Foundation." >> "${output_script_path}"
    ...
    if [ -n "${development}" ]; then
        ...
        echo 'readonly repogpg="https://packages-dev.bongosec.com/key/GPG-KEY-BONGOSEC"' >> "${output_script_path}"
        echo 'readonly repobaseurl="https://packages-dev.bongosec.com/'${devrepo}'"' >> "${output_script_path}"
        ...
        echo 'readonly bucket="packages-dev.bongosec.com"' >> "${output_script_path}"
    ...
    ## JAVA_HOME
    echo "export JAVA_HOME=\"/usr/share/bongosec-indexer/jdk/\"" >> "${output_script_path}"
    ...
    curl -s "https://raw.githubusercontent.com/bongosec/bongosec/${source_branch}/src/init/dist-detect.sh" | sed '/^#/d' >> "${output_script_path}"
    echo "}" >> "${output_script_path}"
...
function buildPasswordsTool() {
    output_script_path="${base_path_builder}/bongosec-passwords-tool.sh"
...
function buildCertsTool() {
    output_script_path="${base_path_builder}/bongosec-certs-tool.sh"
...
function builder_main() {
    ...
    if [ -n "${installer}" ]; then
        ...
        if [ -n "${change_filebeat_url}" ]; then
            sed -i -E "s|(https.+)master(.+bongosec-template.json)|\1\\$\\{source_branch\\}\2|"  "${resources_installer}/installVariables.sh"
        fi
        if [ -n "${development}" ]; then
            sed -i 's|${bongosec_version}|v${bongosec_version}|g' "${resources_installer}/installVariables.sh"
        fi
    fi
...
function checkFilebeatURL() {
    ...
    if [ "${response}" != "200" ]; then
        ...
        if [ "${response}" != "200" ]; then
            echo -e "Error: Could not get the Filebeat Bongosec template. "
        # If matches, replace the variable of installVariables to the new one
        else
            echo -e "Changing Filebeat URL..."
            sed -i -E "s|filebeat_bongosec_template=.*|filebeat_bongosec_template=\"${new_filebeat_url}\"|g" "${resources_installer}/installVariables.sh"
