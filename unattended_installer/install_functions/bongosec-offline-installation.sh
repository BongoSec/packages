...
function offline_extractFiles() {
    ...
    if [ ! -d "${base_path}/bongosec-offline/" ]; then
    ...
    offline_files_path="${base_path}/bongosec-offline/bongosec-files"
    offline_packages_path="${base_path}/bongosec-offline/packages"

    required_files=(
        "${offline_files_path}/filebeat.yml"
        "${offline_files_path}/GPG-KEY-BONGOSEC"
        "${offline_files_path}/bongosec-filebeat-*.tar.gz"
        "${offline_files_path}/bongosec-template.json"
    )
    
    if [ "${sys_type}" == "apt-get" ]; then
        required_files+=("${offline_packages_path}/filebeat-oss-*.deb" "${offline_packages_path}/bongosec-dashboard_*.deb" "${offline_packages_path}/bongosec-indexer_*.deb" "${offline_packages_path}/bongosec-manager_*.deb")
    elif [ "${sys_type}" == "rpm" ]; then
        required_files+=("${offline_packages_path}/filebeat-oss-*.rpm" "${offline_packages_path}/bongosec-dashboard_*.rpm" "${offline_packages_path}/bongosec-indexer_*.rpm" "${offline_packages_path}/bongosec-manager_*.rpm")
    fi
