# Bongosec installer - filebeat.sh functions.
...
function filebeat_configure(){
    ...
    if [ -z "${offline_install}" ]; then
        eval "common_curl -sSo /etc/filebeat/bongosec-template.json ${filebeat_bongosec_template} --max-time 300 --retry 5 --retry-delay 5 --fail"
        if [ ! -f "/etc/filebeat/bongosec-template.json" ]; then
            common_logger -e "Error downloading bongosec-template.json file."
            installCommon_rollBack
            exit 1
        fi
        common_logger -d "Filebeat template was download successfully."

        eval "(common_curl -sS ${filebeat_bongosec_module} --max-time 300 --retry 5 --retry-delay 5 --fail | tar -xvz -C /usr/share/filebeat/module) ${debug}"
        if [ ! -d "/usr/share/filebeat/module" ]; then
            common_logger -e "Error downloading bongosec filebeat module."
            installCommon_rollBack
            exit 1
        fi
        common_logger -d "Filebeat module was downloaded successfully."
    else
        eval "cp ${offline_files_path}/bongosec-template.json /etc/filebeat/bongosec-template.json ${debug}"
        eval "tar -xvzf ${offline_files_path}/bongosec-filebeat-*.tar.gz -C /usr/share/filebeat/module ${debug}"
    fi

    eval "chmod go+r /etc/filebeat/bongosec-template.json ${debug}"
    if [ -n "${AIO}" ]; then
...
function filebeat_copyCertificates() {
    ...
    if [ -f "${tar_file}" ]; then
        if [ -n "${AIO}" ]; then
            ...
            eval "tar -xf ${tar_file} -C ${filebeat_cert_path} --wildcards bongosec-install-files/${server_node_names[0]}.pem --strip-components 1 ${debug}"
            eval "tar -xf ${tar_file} -C ${filebeat_cert_path} --wildcards bongosec-install-files/${server_node_names[0]}-key.pem --strip-components 1 ${debug}"
            eval "tar -xf ${tar_file} -C ${filebeat_cert_path} bongosec-install-files/root-ca.pem --strip-components 1 ${debug}"
            eval "rm -rf ${filebeat_cert_path}/bongosec-install-files/ ${debug}"
        else
            ...
            eval "tar -xf ${tar_file} -C ${filebeat_cert_path} bongosec-install-files/${winame}.pem --strip-components 1 ${debug}"
            eval "tar -xf ${tar_file} -C ${filebeat_cert_path} bongosec-install-files/${winame}-key.pem --strip-components 1 ${debug}"
            eval "tar -xf ${tar_file} -C ${filebeat_cert_path} bongosec-install-files/root-ca.pem --strip-components 1 ${debug}"
            eval "rm -rf ${filebeat_cert_path}/bongosec-install-files/ ${debug}"
        fi
