# Bongosec installer - common.sh functions.
...
function installCommon_addBongosecRepo() {

    common_logger -d "Adding the Bongosec repository."

    if [ -n "${development}" ]; then
        if [ "${sys_type}" == "yum" ]; then
            eval "rm -f /etc/yum.repos.d/bongosec.repo ${debug}"
        elif [ "${sys_type}" == "apt-get" ]; then
            eval "rm -f /etc/apt/sources.list.d/bongosec.list ${debug}"
        fi
    fi

    if [ ! -f "/etc/yum.repos.d/bongosec.repo" ] && [ ! -f "/etc/zypp/repos.d/bongosec.repo" ] && [ ! -f "/etc/apt/sources.list.d/bongosec.list" ] ; then
        if [ "${sys_type}" == "yum" ]; then
            eval "rpm --import ${repogpg} ${debug}"
            if [ "${PIPESTATUS[0]}" != 0 ]; then
                common_logger -e "Cannot import Bongosec GPG key"
                exit 1
            fi
            eval "(echo -e '[bongosec]\ngpgcheck=1\ngpgkey=${repogpg}\nenabled=1\nname=EL-\${releasever} - Bongosec\nbaseurl='${repobaseurl}'/yum/\nprotect=1' | tee /etc/yum.repos.d/bongosec.repo)" "${debug}"
            eval "chmod 644 /etc/yum.repos.d/bongosec.repo ${debug}"
        elif [ "${sys_type}" == "apt-get" ]; then
            eval "common_curl -s ${repogpg} --max-time 300 --retry 5 --retry-delay 5 --fail | gpg --no-default-keyring --keyring gnupg-ring:/usr/share/keyrings/bongosec.gpg --import - ${debug}"
            if [ "${PIPESTATUS[0]}" != 0 ]; then
                common_logger -e "Cannot import Bongosec GPG key"
                exit 1
            fi
            eval "chmod 644 /usr/share/keyrings/bongosec.gpg ${debug}"
            eval "(echo \"deb [signed-by=/usr/share/keyrings/bongosec.gpg] ${repobaseurl}/apt/ ${reporelease} main\" | tee /etc/apt/sources.list.d/bongosec.list)" "${debug}"
            eval "apt-get update -q ${debug}"
            eval "chmod 644 /etc/apt/sources.list.d/bongosec.list ${debug}"
        fi
    else
        common_logger -d "Bongosec repository already exists. Skipping addition."
    fi

    if [ -n "${development}" ]; then
        common_logger "Bongosec development repository added."
    else
        common_logger "Bongosec repository added."
    fi
}
...
function installCommon_changePasswordApi() {
    ...
    if [ -n "${changeall}" ]; then
        for i in "${!api_passwords[@]}"; do
            if [ -n "${bongosec}" ] || [ -n "${AIO}" ]; then
                passwords_getApiUserId "${api_users[i]}"
                BONGOSEC_PASS_API='{\"password\":\"'"${api_passwords[i]}"'\"}'
                eval 'common_curl -s -k -X PUT -H \"Authorization: Bearer $TOKEN_API\" -H \"Content-Type: application/json\" -d "$BONGOSEC_PASS_API" "https://localhost:55000/security/users/${user_id}" -o /dev/null --max-time 300 --retry 5 --retry-delay 5 --fail'
            ...
            if [ "${api_users[i]}" == "bongosec-wui" ] && { [ -n "${dashboard}" ] || [ -n "${AIO}" ]; }; then
        ...
        if [ -n "${bongosec}" ] || [ -n "${AIO}" ]; then
            ...
            BONGOSEC_PASS_API='{\"password\":\"'"${password}"'\"}'
            eval 'common_curl -s -k -X PUT -H \"Authorization: Bearer $TOKEN_API\" -H \"Content-Type: application/json\" -d "$BONGOSEC_PASS_API" "https://localhost:55000/security/users/${user_id}" -o /dev/null --max-time 300 --retry 5 --retry-delay 5 --fail'
        fi
        if [ "${nuser}" == "bongosec-wui" ] && { [ -n "${dashboard}" ] || [ -n "${AIO}" ]; }; then
                passwords_changeDashboardApiPassword "${password}"
        fi
    fi

    for i in "${!api_users[@]}"; do
        if [ "${api_users[i]}" == "bongosec" ] || [ "${api_users[i]}" == "bongosec-wui" ]; then
...
function installCommon_createCertificates() {
    ...
    common_logger -d "Creating Bongosec certificates."
    if [ -n "${AIO}" ]; then
    ...
    if [ -d /tmp/bongosec-certificates/ ]; then
        eval "rm -rf /tmp/bongosec-certificates/ ${debug}"
    fi
    eval "mkdir /tmp/bongosec-certificates/ ${debug}"

    cert_tmp_path="/tmp/bongosec-certificates/"
    ...
    cert_cleanFiles
    eval "chmod 400 /tmp/bongosec-certificates/* ${debug}"
    eval "mv /tmp/bongosec-certificates/* /tmp/bongosec-install-files ${debug}"
    eval "rm -rf /tmp/bongosec-certificates/ ${debug}"
...
function installCommon_createClusterKey() {
    ...
    openssl rand -hex 16 >> "/tmp/bongosec-install-files/clusterkey"

}

function installCommon_createInstallFiles() {

    if [ -d /tmp/bongosec-install-files ]; then
        eval "rm -rf /tmp/bongosec-install-files ${debug}"
    fi

    if eval "mkdir /tmp/bongosec-install-files ${debug}"; then
        ...
        gen_file="/tmp/bongosec-install-files/bongosec-passwords.txt"
        passwords_generatePasswordFile
        eval "cp '${config_file}' '/tmp/bongosec-install-files/config.yml' ${debug}"
        eval "chown root:root /tmp/bongosec-install-files/* ${debug}"
        eval "tar -zcf '${tar_file}' -C '/tmp/' bongosec-install-files/ ${debug}"
        eval "rm -rf '/tmp/bongosec-install-files' ${debug}"
        eval "rm -rf ${config_file} ${debug}"
        common_logger "Created ${tar_file_name}. It contains the Bongosec cluster key, certificates, and passwords necessary for installation."
    else
        common_logger -e "Unable to create /tmp/bongosec-install-files"
        exit 1
    fi
}
...
function installCommon_changePasswords() {
    ...
    if [ -f "${tar_file}" ]; then
        ...
        if { [ -n "${bongosec}" ] || [ -n "${AIO}" ]; } && { [ "${server_node_types[pos]}" == "master" ] || [ "${#server_node_names[@]}" -eq 1 ]; }; then
            passwords_getApiToken
    ...
    if [ -n "${bongosec}" ] || [ -n "${dashboard}" ] || [ -n "${AIO}" ]; then
...
function installCommon_determinePorts {
    ...
    if [ -n "${AIO}" ]; then
        used_ports+=( "${bongosec_aio_ports[@]}" )
    elif [ -n "${bongosec}" ]; then
        used_ports+=( "${bongosec_manager_ports[@]}" )
    elif [ -n "${indexer}" ]; then
        used_ports+=( "${bongosec_indexer_ports[@]}" )
    elif [ -n "${dashboard}" ]; then
        used_ports+=( "${bongosec_dashboard_port[@]}" )
    fi
}

function installCommon_extractConfig() {

    common_logger -d "Extracting Bongosec configuration."
    if ! tar -tf "${tar_file}" | grep -q bongosec-install-files/config.yml; then
        common_logger -e "There is no config.yml file in ${tar_file}."
        exit 1
    fi
    eval "tar -xf ${tar_file} -C /tmp bongosec-install-files/config.yml ${debug}"
...
function installCommon_installDependencies() {
    ...
    if [ "${1}" == "assistant" ]; then
        ...
        installCommon_installList "${bongosec_deps_to_install[@]}"
    fi
}
...
For Bongosec indexer users, the file must have this format:

# Description
  indexer_username: <user>
  indexer_password: <password>

For Bongosec API users, the file must have this format:
...
"
    ...
    if [ -n "${changeall}" ]; then
        ...
        for j in "${!fileapiusers[@]}"; do
            ...
            if [ "${supported}" = false ] && [ -n "${indexer_installed}" ]; then
                common_logger -e "The Bongosec API user ${fileapiusers[j]} does not exist"
            fi
        done
    else
        finalusers=()
        ...
        if [ -n "${filebeat_installed}" ] && [ -n "${bongosec}" ]; then
            users=( admin )
        fi
...
function installCommon_restoreBongosecrepo() {

    common_logger -d "Restoring Bongosec repository."
    if [ -n "${development}" ]; then
        if [ "${sys_type}" == "yum" ] && [ -f "/etc/yum.repos.d/bongosec.repo" ]; then
            file="/etc/yum.repos.d/bongosec.repo"
        elif [ "${sys_type}" == "apt-get" ] && [ -f "/etc/apt/sources.list.d/bongosec.list" ]; then
            file="/etc/apt/sources.list.d/bongosec.list"
        else
            common_logger -w -d "Bongosec repository does not exists."
        fi
...
function installCommon_rollBack() {
    ...
    if [ -z "${uninstall}" ]; then
        common_logger "--- Removing existing Bongosec installation ---"
    fi

    if [ -f "/etc/yum.repos.d/bongosec.repo" ]; then
        eval "rm /etc/yum.repos.d/bongosec.repo ${debug}"
    elif [ -f "/etc/zypp/repos.d/bongosec.repo" ]; then
        eval "rm /etc/zypp/repos.d/bongosec.repo ${debug}"
    elif [ -f "/etc/apt/sources.list.d/bongosec.list" ]; then
        eval "rm /etc/apt/sources.list.d/bongosec.list ${debug}"
    fi
    ...
    if [[ -n "${bongosec_installed}" && ( -n "${bongosec}" || -n "${AIO}" || -n "${uninstall}" ) ]];then
        common_logger "Removing Bongosec manager."
        if [ "${sys_type}" == "yum" ]; then
            common_checkYumLock
            if [ "${attempt}" -ne "${max_attempts}" ]; then
                eval "yum remove bongosec-manager -y ${debug}"
                eval "rpm -q bongosec-manager --quiet"
            fi
        elif [ "${sys_type}" == "apt-get" ]; then
            common_checkAptLock
            eval "apt-get remove --purge bongosec-manager -y ${debug}"
            eval "dpkg -l bongosec-manager 2>/dev/null | grep -E '^ii\s'"
        fi

        manager_installed=${PIPESTATUS[0]}

        if [ "${manager_installed}" -eq 0 ]; then
            common_logger -w "The Bongosec manager package could not be removed."
        else
            common_logger "Bongosec manager removed."
        fi

    fi

    if [[ ( -n "${bongosec_remaining_files}"  || -n "${bongosec_installed}" ) && ( -n "${bongosec}" || -n "${AIO}" || -n "${uninstall}" ) ]]; then
        eval "rm -rf /var/ossec/ ${debug}"
    fi

    if [[ -n "${indexer_installed}" && ( -n "${indexer}" || -n "${AIO}" || -n "${uninstall}" ) ]]; then
        common_logger "Removing Bongosec indexer."
        if [ "${sys_type}" == "yum" ]; then
            common_checkYumLock
            if [ "${attempt}" -ne "${max_attempts}" ]; then
                eval "yum remove bongosec-indexer -y ${debug}"
                eval "rpm -q bongosec-indexer --quiet"
            fi
        elif [ "${sys_type}" == "apt-get" ]; then
            common_checkAptLock
            eval "apt-get remove --purge bongosec-indexer -y ${debug}"
            eval "dpkg -l bongosec-indexer 2>/dev/null | grep -E '^ii\s'"
        fi

        indexer_installed=${PIPESTATUS[0]}

        if [ "${indexer_installed}" -eq 0 ]; then
            common_logger -w "The Bongosec indexer package could not be removed."
        else
            common_logger "Bongosec indexer removed."
        fi
    fi

    if [[ ( -n "${indexer_remaining_files}" || -n "${indexer_installed}" ) && ( -n "${indexer}" || -n "${AIO}" || -n "${uninstall}" ) ]]; then
        eval "rm -rf /var/lib/bongosec-indexer/ ${debug}"
        eval "rm -rf /usr/share/bongosec-indexer/ ${debug}"
        eval "rm -rf /etc/bongosec-indexer/ ${debug}"
    fi

    if [[ -n "${filebeat_installed}" && ( -n "${bongosec}" || -n "${AIO}" || -n "${uninstall}" ) ]]; then
    ...
    if [[ ( -n "${filebeat_remaining_files}" || -n "${filebeat_installed}" ) && ( -n "${bongosec}" || -n "${AIO}" || -n "${uninstall}" ) ]]; then
    ...
    if [[ -n "${dashboard_installed}" && ( -n "${dashboard}" || -n "${AIO}" || -n "${uninstall}" ) ]]; then
        common_logger "Removing Bongosec dashboard."
        if [ "${sys_type}" == "yum" ]; then
            common_checkYumLock
            if [ "${attempt}" -ne "${max_attempts}" ]; then
                eval "yum remove bongosec-dashboard -y ${debug}"
                eval "rpm -q bongosec-dashboard --quiet"
            fi
        elif [ "${sys_type}" == "apt-get" ]; then
            common_checkAptLock
            eval "apt-get remove --purge bongosec-dashboard -y ${debug}"
            eval "dpkg -l bongosec-dashboard 2>/dev/null | grep -E '^ii\s'"
        fi

        dashboard_installed=${PIPESTATUS[0]}

        if [ "${dashboard_installed}" -eq 0 ]; then
            common_logger -w "The Bongosec dashboard package could not be removed."
        else
            common_logger "Bongosec dashboard removed."
        fi
    fi

    if [[ ( -n "${dashboard_remaining_files}" || -n "${dashboard_installed}" ) && ( -n "${dashboard}" || -n "${AIO}" || -n "${uninstall}" ) ]]; then
        eval "rm -rf /var/lib/bongosec-dashboard/ ${debug}"
        eval "rm -rf /usr/share/bongosec-dashboard/ ${debug}"
        eval "rm -rf /etc/bongosec-dashboard/ ${debug}"
        eval "rm -rf /run/bongosec-dashboard/ ${debug}"
    fi

    elements_to_remove=(    "/var/log/bongosec-indexer/"
                            "/var/log/filebeat/"
                            "/etc/systemd/system/opensearch.service.wants/"
                            "/securityadmin_demo.sh"
                            "/etc/systemd/system/multi-user.target.wants/bongosec-manager.service"
                            "/etc/systemd/system/multi-user.target.wants/filebeat.service"
                            "/etc/systemd/system/multi-user.target.wants/opensearch.service"
                            "/etc/systemd/system/multi-user.target.wants/bongosec-dashboard.service"
                            "/etc/systemd/system/bongosec-dashboard.service"
...
function installCommon_scanDependencies() {
    ...
    if [ -n "${AIO}" ]; then
        if [ "${sys_type}" == "yum" ]; then
            bongosec_deps+=( "${indexer_yum_dependencies[@]}" "${bongosec_yum_dependencies[@]}" "${dashboard_yum_dependencies[@]}" )
        else 
            bongosec_deps+=( "${indexer_apt_dependencies[@]}" "${bongosec_apt_dependencies[@]}" "${dashboard_apt_dependencies[@]}" )
        fi
    elif [ -n "${indexer}" ]; then
        if [ "${sys_type}" == "yum" ]; then
            bongosec_deps+=( "${indexer_yum_dependencies[@]}" )
        else 
            bongosec_deps+=( "${indexer_apt_dependencies[@]}" )
        fi
    elif [ -n "${bongosec}" ]; then
        if [ "${sys_type}" == "yum" ]; then
            bongosec_deps+=( "${bongosec_yum_dependencies[@]}" )
        else 
            bongosec_deps+=( "${bongosec_apt_dependencies[@]}" )
        fi
    elif [ -n "${dashboard}" ]; then
        if [ "${sys_type}" == "yum" ]; then
            bongosec_deps+=( "${dashboard_yum_dependencies[@]}" )
        else 
            bongosec_deps+=( "${dashboard_apt_dependencies[@]}" )
        fi
    fi

    all_deps=( "${bongosec_deps[@]}" )
    if [ "${sys_type}" == "apt-get" ]; then
    ...
    if [ -z "${AIO}" ] && [ -z "${bongosec}" ] && [ -z "${indexer}" ] && [ -z "${dashboard}" ]; then
    ...
    # Get not installed dependencies of Assistant and Bongosec
