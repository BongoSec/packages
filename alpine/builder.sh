...
if [ "${local_source}" = "no" ]; then
    curl -sL https://github.com/bongosec/bongosec/tarball/${reference} | tar zx
fi
export version="$(cat bongosec*/src/VERSION | cut -d 'v' -f 2)"

if [ "${future}" = "yes" ]; then
    old_version=$version
    MAJOR=$(echo $version | cut -dv -f2 | cut -d. -f1)
    export version="${MAJOR}.30.0"
    sed -i "s/${old_version}/${version}/g" "/bongosec"*"/src/init/bongosec-server.sh"
    sed -i "s/${old_version}/${version}/g" "/bongosec"*"/src/init/bongosec-client.sh"
    sed -i "s/${old_version}/${version}/g" "/bongosec"*"/src/init/bongosec-local.sh"
fi
...
if [ "${local_spec}" = "no" ]; then
    curl -sL https://github.com/bongosec/packages/tarball/${spec_reference} | tar zx
    cp -r ./bongosec*/alpine/SPECS/${target} ${spec_path}/
else
...
cd ${pkg_path} && sha512sum ${pkg_file} > /var/local/bongosec/${pkg_file}.sha512
find ${pkg_path}/ -maxdepth 3 -type f -name "${file_name}*" -exec mv {} /var/local/bongosec/ \;
