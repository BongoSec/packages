"""
This script is used to bump the version of the Bongosec packages repository.
    Copyright (C) 2015-2020, Bongosec Inc.
...
spec_files_dict = {
    ...
    r'%changelog':'%changelog\n' 
        + (f"* {spec_date} support <info@bongosec.com> - {version}"
        "\n- More info: https://documentation.bongosec.com/current/release-"
        f"notes/release-{version.major}-{version.minor}-"
        f"{version.micro}.html")}

copyright_files_dict = {
    (r'Bongosec, Inc <info@bongosec.com> on '
     r'(\w+),\s(\d+)\s(\w+)\s(\d+)\s(\d+):(\d+):(\d+)\s\+(\d+)'):
     f"Bongosec, Inc <info@bongosec.com> on {deb_changelog_date}"}
...
pkgproj_files_dict = {
    ...
    r'<string>bongosec-agent-(\d+\.\d+\.\d+)-(\d+)':
    f'<string>bongosec-agent-{version}-{args.revision}'}

test_files_dict = {
    r'bongosec-manager.x86_64\s+(\d+\.\d+\.\d+)-(\d+)':
    f'bongosec-manager.x86_64 {version}-{args.revision}',
    r'bongosec_version=\"(\d+\.\d+\.\d+)\"':
    f'bongosec_version=\"{version}\"'}

install_variables_files_dict = {
    r'bongosec_major=\"(\d+\.\d+)\"':
    f'bongosec_major=\"{version.major}.{version.minor}\"',
    r'bongosec_version=\"(\d+\.\d+\.\d+)\"':f'bongosec_version=\"{version}\"'}
...
changelog_md_files_dict = {
    ...
    (r'All notable changes to this project '
    r'will be documented in this file.') + '\n'  
    + (f"## [{version}]\n\n- https://github.com/bongosec/"
    f"packages/releases/tag/v{version}\n")}
...
for changelog_file in changelog_files:
    with open(changelog_file, 'r', encoding="utf-8") as file:
        ...
        filedata=file.read()
        install_type=re.search(r'(bongosec-(agent|manager|indexer|dashboard))',
                               filedata).group(1)
        changelog_string=(f"{install_type} ({version}-RELEASE) stable; "
            "urgency=low\n\n  * More info: https://documentation.bongosec.com/"
            f"current/release-notes/release-{version.major}-{version.minor}-"
            f"{version.micro}.html\n\n -- "
            f"Bongosec, Inc <info@bongosec.com>  {deb_changelog_date}\n\n")
