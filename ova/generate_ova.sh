...
# Program to build the Bongosec Virtual Machine
...
help () {
    echo -e ""
    echo -e "NAME"
    echo -e "$(basename "$0") - Build Bongosec OVA."
    echo -e ""
    echo -e "SYNOPSIS"
...
clean() {
    ...
    cd "${scriptpath}"
    vagrant destroy -f
    OVA_VMDK="bongosec-${OVA_VERSION}-disk001.vmdk"
...
build_ova() {
    ...
    OVA_VM="bongosec-${OVA_VERSION}.ova"
    OVF_VM="bongosec-${OVA_VERSION}.ovf"
    OVA_FIXED="bongosec-${OVA_VERSION}-fixed.ova"
    ...
    # Get machine name
    VM_EXPORT=$(vboxmanage list vms | grep -i vm_bongosec | cut -d "\"" -f2)
...
main() {
    ...
    # Standarize OVA
    bash setOVADefault.sh "${scriptpath}" "${OUTPUT_DIR}/${OVA_VM}" "${OUTPUT_DIR}/${OVA_VM}" "${scriptpath}/bongosec_ovf_template" "${OVA_VERSION}"  || clean 1
