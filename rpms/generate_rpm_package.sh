...
TAR_URL="https://packages-dev.bongosec.com/utils/centos-5-i386-build/centos-5-i386.tar.gz"
...
clean() {
    ...
    # Clean the files
    rm -rf ${DOCKERFILE_PATH}/{*.tar.gz,bongosec*} ${DOCKERFILE_PATH}/build.sh ${SOURCES_DIRECTORY}
...
build_rpm() {
    ...
    if [ ! -z "${LOCAL_SOURCE_CODE}" ]; then
        CUSTOM_CODE_VOL="-v ${LOCAL_SOURCE_CODE}:/bongosec-local-src:Z"
...
help() {
    ...
    echo "    --sources <path>             [Optional] Absolute path containing bongosec source code. This option will use local source code instead of downloading it from GitHub."
    echo "    --packages-branch <branch>   [Optional] Select Git branch or tag from packages repository. e.g ${PACKAGES_BRANCH}"
