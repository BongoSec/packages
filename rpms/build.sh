...
cp -R bongosec-* ${build_dir}/${package_name}
...
if [ "${use_local_specs}" = "no" ]; then
    ...
    curl -L "https://raw.githubusercontent.com/bongosec/packages/${bongosec_packages_branch}/rpms/SPECS/bongosec-${build_target}.spec" -o "${specs_path}/bongosec-${build_target}.spec" ${ssl_flag}
else
    specs_path="/specs"
fi
...
if [[ "${future}" == "yes" ]]; then
    ...
    find "${build_dir}/${package_name}" "${specs_path}/" \( -name "*VERSION*" -o -name "*.spec" \) -exec sed -i "s/${base_version}/${bongosec_version}/g" {} \;
    sed -i "s/\$(VERSION)/${MAJOR}.${MINOR}/g" "${build_dir}/${package_name}/src/Makefile"
    sed -i "s/${base_version}/${bongosec_version}/g" "${build_dir}/${package_name}/src/init/bongosec-server.sh"
    sed -i "s/${base_version}/${bongosec_version}/g" "${build_dir}/${package_name}/src/init/bongosec-client.sh"
    sed -i "s/${base_version}/${bongosec_version}/g" "${build_dir}/${package_name}/src/init/bongosec-local.sh"
fi
...
find ${extract_path} -maxdepth 3 -type f -name "${file_name}*" -exec mv {} /var/local/bongosec \;
