...
if(($help.isPresent)) {
    ...
    PARAMETERS TO BUILD BONGOSEC-AGENT MSI:
    ...
    USAGE:
        ...
        * BONGOSEC:
          $ ./generate_bongosec_msi.ps1  -OPTIONAL_REVISION {{ REVISION }} -SIGN {{ yes|no }} -WIX_TOOLS_PATH {{ PATH }} -SIGN_TOOLS_PATH {{ PATH }}

            Build a devel msi:    $ ./generate_bongosec_msi.ps1 -OPTIONAL_REVISION 2 -SIGN no
            Build a prod msi:     $ ./generate_bongosec_msi.ps1 -OPTIONAL_REVISION 1 -SIGN yes -
    "
    Exit
}
...
function ComputeMsiName() {
    ...
    $MSI_NAME="bongosec-agent-$VERSION-$OPTIONAL_REVISION.msi"
    return $MSI_NAME
}
...
function BuildBongosecMsi(){
    ...
    if($SIGN -eq "yes"){
        ...
        & $SIGNTOOL_EXE sign /a /tr http://timestamp.digicert.com /fd SHA256 /td SHA256 "..\bongosec_modules\syscollector\build\bin\syscollector.dll"
